...
class AutenticacionUseCaseTest {
    ...
    void setUp() {
        ...
        autenticacionUseCase = new AutenticacionUseCase(autenticacionRepository, jwtTokenService);
    ...
    class AutenticacionExitosa {
        ...
        void debeAutenticarUsuarioConCredencialesValidas() {
            ...
            when(autenticacionRepository.findUserByEmailForAuthentication(EMAIL_VALIDO))
                    .thenReturn(Mono.just(usuarioCompleto));
            when(autenticacionRepository.validateUserPassword(EMAIL_VALIDO, PASSWORD_VALIDO))
                    .thenReturn(Mono.just(true));
            when(jwtTokenService.generateToken(any(TokenClaims.class)))
                    .thenReturn(Mono.just(jwtToken));

            StepVerifier.create(autenticacionUseCase.authenticate(EMAIL_VALIDO, PASSWORD_VALIDO))
                    .assertNext(token -> {
            ...
            verify(autenticacionRepository).findUserByEmailForAuthentication(EMAIL_VALIDO);
            verify(autenticacionRepository).validateUserPassword(EMAIL_VALIDO, PASSWORD_VALIDO);
    ...
    class ValidacionCredencialesFallida {
        ...
        void debeFallarCuandoEmailNoExiste() {
            ...
            verify(autenticacionRepository).findUserByEmailForAuthentication(EMAIL_VALIDO);
            verify(autenticacionRepository, never()).validateUserPassword(anyString(), anyString());
        ...
        void debeContenerEmailEnMensajeDeErrorCuandoNoExiste() {
            ...
            StepVerifier.create(autenticacionUseCase.authenticate(EMAIL_VALIDO, PASSWORD_VALIDO))
                    .expectErrorMatches(error -> 
                            ...
                            ((BusinessException) error).getErrorCode() == ErrorCode.CORREO_NOT_EXISTS
                    )
                    .verify();
        }
        ...
        void debeRetornarErrorEspecificoParaPasswordInvalido() {
            ...
            StepVerifier.create(autenticacionUseCase.authenticate(EMAIL_VALIDO, PASSWORD_VALIDO))
                    .expectErrorMatches(error -> 
                            error instanceof BusinessException &&
                            ((BusinessException) error).getErrorCode() == ErrorCode.PASSWORD_INVALID
                    )
                    .verify();
        }
    }
    ...
    class ValidacionToken {
        ...
        void debeValidarTokenCorrectamenteCuandoEsValido() {
            ...
            StepVerifier.create(autenticacionUseCase.validateToken(jwtToken))
                    .expectNext(true)
                    .verifyComplete();
    ...
    class ExtraccionClaims {
        ...
        void debeExtraerClaimsDelTokenCorrectamente() {
            ...
            StepVerifier.create(autenticacionUseCase.extractClaims(jwtToken))
                    .assertNext(claims -> {
